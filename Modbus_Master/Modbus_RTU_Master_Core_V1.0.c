/*
******************************************************************************************************************
*Filename      :Modbus_RTU_Master_Core.c
*Programmer(s) :Nrush
*Description   :该文件为Modbus RTU主机通讯核心代码与硬件平台无关，功能完整。
				测试平台为stm32f103rct6,推荐使用至少可产生50us中断的单片机。
******************************************************************************************************************
*/
/* $ PAGE*/
#include "Modbus_RTU_Master_Port_V1.0.h"		//对外接口，生成LIB文件后必须包含该文件

#ifdef MODBUS_DEBUG						//如果启用调试模式，将需要调用输出函数对外输出相关信息
	#include "stdio.h"
#endif
/*
******************************************************************************************************************
...
*******************************************************************************************************************
*/
void Modbus_Add_FC_List_Item(modbus_st* modbus,modbus_fc_list_st* add_item,uint8 fc_id,void (*fc_m_rx)(void* ptr))
{
...
/******************************************************************************************************************
*                                   void Modbus_Clear_RX_Buf(modbus_st *modbus)
...
/******************************************************************************************************************
*                                  void Modbus_Clear_TX_Buf(uint8 *buf,uint16 length)
*Description : 清除Modbus发送缓存
*Arguments   : buf 发送缓存区地址 length 清除长度
*Returns     : None
*Notes       : None
*******************************************************************************************************************
*/
...
*Description : 从机发送数据
*Arguments   : modbus Modbus控制器的句柄 Outlength 发送长度
*Returns     : None
*Notes       : None
*******************************************************************************************************************
*/
...
{  
	...
	(*modbus).rx_buf[Outlength+1] = temp&0xFF;
	(*modbus).rx_buf[Outlength] = (temp&0xFF00) >> 8;
	...
	Modbus_Clear_TX_Buf((*modbus).rx_buf,Outlength + 2);
}
/******************************************************************************************************************
*                                   void Modbus_Reset_Slaver(modbus_st* modbus)
*Description : 重置Modbus从机状态
*Arguments   : modbus Modbus控制器的句柄
*Returns     : None
*Notes       : None
*******************************************************************************************************************
...
*Description : Modbus更新设置参数
*Arguments   : modbus Modbus控制器的句柄
*Returns     : None
*Notes       : 参数设置后使用该函数刷新后才会生效
*******************************************************************************************************************
*/
void Modbus_Refresh_Set(modbus_st* modbus)
{
	uint16 temp = 0;
...
void Modbus_Init(modbus_st* modbus)
{
	...
	Modbus_List_Init(modbus);
}

/******************************************************************************************************************
*                           void Modbus_Receive_Data_IRQ(modbus_st* modbus,uint8 data)
*Description : 中断接收数据
*Arguments   : modbus Modbus控制器的句柄 data 接收到的数据
*Returns     : None
*Notes       : 该函数需要在串口接收中断中使用
*******************************************************************************************************************
*/
void Modbus_Receive_Data_IRQ(modbus_st* modbus,uint8 data)
{
...
/******************************************************************************************************************
*                                   void BE_Check(modbus_st *modbus)	
*Description : 帧头尾格式检测
*Arguments   : modbus Modbus控制器的句柄 
*Returns     : None
*Notes       : 若有错误会更新错误标志
*******************************************************************************************************************
*/
static void BE_Check(modbus_st *modbus)	
{
	unsigned int temp = 0;
	...
	{
		if(((temp&0xFF) == (*modbus).rx_buf[(*modbus).rx_num - 1]) && (((temp&0xFF00)>>8) == (*modbus).rx_buf[(*modbus).rx_num - 2]))	/*CRC校验*/
		{
			(*modbus).error_info = MODBUS_NO_ERROR;
		}
		else
...
/******************************************************************************************************************
*                                   static void FC_Realized(modbus_st *modbus)
*Description : 功能码功能实现
*Arguments   : modbus Modbus控制器的句柄
*Returns     : None
*Notes       : None
...
*Description : Modbus接收处理解码函数
*Arguments   : modbus Modbus控制器的句柄
*Returns     : None
*Notes       : 该函数需要在主循环或者定时中断中使用以用来解析数据
*******************************************************************************************************************
*/
void Modbus_Receive_Data(modbus_st* modbus)
{
...
*Description : 读取输入位寄存器值
*Arguments   : base_add 基地址 length 长度 buf 读入缓冲区
...
boolean Modbus_Read_HoldBit_Data(uint16 base_add,uint16 length,uint8* buf)
{
	...
	for(i=0;i<length;i++)
	{
		...
		{
#ifdef MODBUS_DEBUG		
			printf("MODBUS_READ_HOLDBIT_ERR\r\n");
#endif
			return MODBUS_ON;
		}					
	}
...
*Description : 写入保持位寄存器值
*Arguments   : base_add 基地址 length 长度 buf 待写入数据缓冲区
...
boolean Modbus_Write_HoldBit_Data(uint16 base_add,uint16 length,uint8* buf)
{
	...
	for(i=0;i<length;i++)
	{
		...
		{
#ifdef MODBUS_DEBUG		
			printf("MODBUS_WRITE_HOLDBIT_ERR\r\n");
#endif
			return MODBUS_ON;			
		}		
	}
...
boolean Modbus_Read_In2Bytes_Data(uint16 base_add,uint16 length,uint8* buf)
{
	...
	for(i=0;i<length;i++)
	{
		...
		{
#ifdef MODBUS_DEBUG		
			printf("MODBUS_READ_IN2BYTES_ERR\r\n");
#endif
			return MODBUS_ON;
		}
	}
...
boolean Modbus_Read_Hold2Bytes_Data(uint16 base_add,uint16 length,uint8* buf)
{
	...
	for(i=0;i<length;i++)
	{
		...
		{
#ifdef MODBUS_DEBUG		
			printf("MODBUS_READ_HOLDBYTES_ERR\r\n");
#endif
			return MODBUS_ON;
		}
	}
...
boolean Modbus_Write_Hold2Bytes_Data(uint16 base_add,uint16 length,uint8* buf)
{
	...
	for(i=0;i<length;i++)
	{
		...
		{
#ifdef MODBUS_DEBUG		
			printf("MODBUS_WRITE_HOLDBYTES_ERR\r\n");
#endif
			return MODBUS_ON;			
		}		
	}
