...
#define MODBUS_DEFAULT_BOUND					MODBUS_USART_BAUDRATE_9600			/* 默认波特率*/
...
#define MODBUS_TIMER_TICK						((uint16)1000)						/* 定时器计数频率kHz,至少25kHz，最高不大于1MHz*/
#define MODBUS_TIMER_MAX						((uint16)5000)						/* 定时器时钟最大计数时间us*/

/* Modbus储存功能支持选择，置0则启用，置1则启用*/
#define MODBUS_STORE                         (1)                                    /* Modbus数据储存功能开关*/
#define MODBUS_PARAMETER_BASE_ADDRESS			((uint32)0x00)						/* Modbus数据保存基地址，为实际物理地址*/
...
#define MODBUS_FC01  ((uint8)01)
#define MODBUS_FC02  ((uint8)02)
#define MODBUS_FC03  ((uint8)03)
#define MODBUS_FC04  ((uint8)04)
#define MODBUS_FC05  ((uint8)05)
#define MODBUS_FC06  ((uint8)06)
#define MODBUS_FC15  ((uint8)15)
#define MODBUS_FC16  ((uint8)16)
/*
******************************************************************************************************************
*                                           STRUCT & MACRO
...
#if MODBUS_TIMER_MAX < 2292
	uint8 timer_overflow_counter;
#endif
...
#define MODBUS_USART_28800_HBT_US					((uint16)250)
#define MODBUS_USART_BAUDRATE_38400					((uint8)0x18)
#define MODBUS_USART_38400_HBT_US					((uint16)250)
#define MODBUS_USART_BAUDRATE_57600					((uint8)0x1C)
#define MODBUS_USART_57600_HBT_US					((uint16)250)
#define MODBUS_USART_BAUDRATE_115200				((uint8)0x20)
#define MODBUS_USART_115200_HBT_US					((uint16)250)
...
{
	uint8	fc_id;									/*功能码*/
	void (*fc_m_rx)(void* ptr);						/*主机功能码处理函数*/
	struct fc_list* next;
}modbus_fc_list_st;
...
boolean Modbus_Write_Hold2Bytes_Port(uint16 add,uint16 dat);	/*读取16位保持寄存器数据*/
