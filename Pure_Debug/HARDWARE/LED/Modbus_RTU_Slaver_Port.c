...
void Modbus_FC01_SRX(void* ptr)
{
   ...
   (*modbus).rx_buf[1] = cmd;
	...
	if((address + num - 1) > (MODBUS_HOLDBIT_REG_START+MODBUS_HOLDBIT_REG_MAX_LENGTH)\
   ...
   if((address + num - 1) > (MODBUS_HOLDBIT_REG_START+MODBUS_HOLDBIT_REG_MAX_LENGTH))
	...
	else if(num > MODBUS_MAX_RW_BITS)
	{
...
//static modbus_fc_list_st Read_Bit_FC3;

void Modbus_List_Init_Port(modbus_st* modbus)			/*用来重置并开启定时器及其中断的函数*/
{
	Modbus_Add_FC_List_Item(modbus,&modbus_FC01,0x01,Modbus_FC01_SRX);	
	Modbus_Add_FC_List_Item(modbus,&modbus_FC02,0x02,Modbus_FC02_SRX);
}
