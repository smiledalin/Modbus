...
*******************************************************************************************************************
*/
void Clear_Buf(uint8 *buf,uint16 length)
{
	uint16 i=0;
	for(i = 0;i<length;i++)
	{
...
void Modbus_Error_Slove(modbus_st* modbus)
{
	...
	else if((*modbus).error_info == MODBUS_SLAVER_NO_FC_ERR)
	{
		Modbus_Reset_Slaver(modbus);	
		printf("MODBUS_SLAVER_NO_FC_ERR\r\n");
	}
...
static void FC_Realized(modbus_st *modbus)	/*功能码功能实现*/
{
	...
	{
		 /*非法功能码否异常*/
		(*modbus).error_info = MODBUS_SLAVER_NO_FC_ERR;
	}	
