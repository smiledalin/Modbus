...
{
	Modbus_Default_Parameter(modbus);
	Modbus_Load_Parameter(modbus);		//从储存器中加载Modbus相关参数
 Modbus_Usart_Init((*modbus).usart);	//初始化Modbus串口
	Modbus_Timer_Init((*modbus).timer);	//初始化Modbus专用定时器	
	Modbus_List_Init();
}
/******************************************************************************************************************
*                                   void Modbus_Default_Parameter(modbus_st* modbus)
...
static void BE_Check(modbus_st *modbus)	/*帧头尾格式检测*/
{
  ...
  || (((*modbus).rx_buf[0] == (*modbus).config.dev_address)&&((*modbus).config.dev_mode == MODBUS_SLAVER)))	/*地址校验*/
	{
...
static void FC_Realized(modbus_st *modbus)	/*功能码功能实现*/
{
	...
	{
		if((*modbus).rx_buf[1] == (*modbus).tx_buf[1])	/*检测功能码返回是否异常*/
		{
			/*功能码返回正常*/
...
void Modbus_Receive_Data(modbus_st* modbus)
{
	if((*modbus).config.modbus_mode == MODBUS_RTU)
	{
		if((*modbus).config.dev_mode == MODBUS_MASTER)
		{
			...
			if((*modbus).status != MODBUS_FREE)
			{
				if((*modbus).status == MODBUS_TX)	/*第一阶段测试返回时间是否超时*/
				{
					while(((*modbus).status == MODBUS_TX) && ((*modbus).error_info == MODBUS_NO_ERROR)); //这里为了保证实时性使用循环查询
				...
				else if((*modbus).status == MODBUS_RX)	/*第二阶段测试接收是否完成或是否发生超时错误*/
				{
					while((((*modbus).timer.timeout_flag == MODBUS_TIMER_NOTIMEOUT)||((*modbus).timer.timeout_flag == MODBUS_TIMER_BYTE_OUT))\
						&& ((*modbus).error_info == MODBUS_NO_ERROR)); //这里为了保证实时性使用循环查询
