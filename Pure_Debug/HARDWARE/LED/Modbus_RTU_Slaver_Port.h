...
#define MODBUS_HOLDBIT_REG_START							(0)
#define MODBUS_HOLDBIT_REG_MAX_LENGTH						(32)

#define	MODBUS_INBIT_REG_BASE_ADDRESS						()
#define MODBUS_INBIT_REG_START								(0)
#define MODBUS_INBIT_REG_MAX_LENGTH							(32)

#define	MODBUS_HOLDBYTE_REG_BASE_ADDRESS					()
#define MODBUS_HOLDBYTE_REG_START							(0)
#define MODBUS_HOLDBYTE_REG_MAX_LENGTH						(2)

#define	MODBUS_INBYTE_REG_BASE_ADDRESS						()
#define MODBUS_INBYTE_REG_START								(0)
#define MODBUS_INBYTE_REG_MAX_LENGTH						(2)

#define MODBUS_MAX_RW_BITS									(2000)
#define MODBUS_MAX_RW_2BYTES								(2000)
...
extern void Clear_TX_Buf(uint8 *buf,uint16 length);
extern void Modbus_Error_Slove(modbus_st* modbus);
extern boolean Modbus_Read_InBit_Data(uint16 base_add,uint16 length,uint8* buf);
extern boolean Modbus_Read_HoldBit_Data(uint16 base_add,uint16 length,uint8* buf);
extern boolean Modbus_Write_HoldBit_Data(uint16 base_add,uint16 length,uint8* buf);
extern boolean Modbus_Read_In2Bytes_Data(uint16 base_add,uint16 length,uint8* buf);
extern boolean Modbus_Read_Hold2Bytes_Data(uint16 base_add,uint16 length,uint8* buf);
extern boolean Modbus_Write_Hold2Bytes_Data(uint16 base_add,uint16 length,uint8* buf);
